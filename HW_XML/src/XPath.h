...
private:
	...
	wchar_t* cmdBuf; // 커맨드 한 줄의 버퍼
	wchar_t* strBuf; // 커맨드 한 줄에서 일정한 기준으로 끊어낸 커맨드 저장용 버퍼
...
public:
	...
	void ErrorCollection(const wchar_t* str); //XPathCmdParser() 함수 호출중에 발생하는 error들을 한곳에서 처리함.
	void FuncCollection(const wchar_t* str); //커맨드 중에 함수 호출이 있을 경우 여기서 처리함.

	int XPathCmdParser(wchar_t* _cmdBuf, XMLNode* _XpathRoute);//cmd분석 함수.
	...
	void Search_All(XMLNode* _XpathRoute, const wchar_t* str, CommandType _commandType);//가장 기본적인 탐색함수. 전체 순회 방식
	void Search_Child(const wchar_t* str); //searchNodeQ 큐에 있는 노드들의 자식중에 문자열str과 일치하는 노드들을 push함.
