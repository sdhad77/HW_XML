...
XPath::XPath() {
	...
	cmdIdx = 0;
	cmdBuf = new wchar_t[MAX_BUF_SIZE];
	strBuf = new wchar_t[MAX_CHAR_SIZE];
}
...
void XPath::ErrorCollection(const wchar_t* str)
{
	if(!wcscmp(str, L"cmd"))		wprintf(L"cmd를 다시 입력해 주세요.\n");
	else if(!wcscmp(str, L"//@"))	wprintf(L"잘못입력하였습니다.\n");
	else if(!wcscmp(str, L"//"))	wprintf(L"// 이후 알파벳,*,@ 이외의 문자가 입력되었습니다.\n");
	else if(!wcscmp(str, L"/"))		wprintf(L"/ 이후 처리되지 않은 문자가 나타났습니다.\n");
	else if(!wcscmp(str, L"[999]"))	wprintf(L"입력된 숫자가 너무 큽니다.\n");
	else if(!wcscmp(str, L"[999"))	wprintf(L"[로 시작하면 ]로 끝나야 합니다.\n");
	else if(!wcscmp(str, L"[func("))wprintf(L") 는 어디에..\n");
	else if(!wcscmp(str, L"[~"))	wprintf(L"[ 이후 처리되지 않은 문자가 입력되었습니다.\n");
	else if(!wcscmp(str, L"[name~$"))wprintf(L"[a 이후 처리되지 않은 문자가 입력되었습니다.\n");
	else if(!wcscmp(str, L"FuncName"))wprintf(L"아직 처리 되지 않은 함수명이 입력되었습니다.\n");
...
void XPath::Search_All(XMLNode* _XpathRoute, const wchar_t* str, CommandType _commandType)
{
	std::list<XMLNode>::iterator _iter;
	...
	{
		if(!wcscmp(_XpathRoute->getName(), str)) searchNodeQ.push(_XpathRoute);
	}
	...
	else if(_commandType == search_AttributeName)
	{
		...
		{
			if(!wcscmp(_iter2->getName(), str)) searchNodeQ.push(&(*_iter2));
		}
	}
...
void XPath::Search_Child(const wchar_t* str)
{
	std::list<XMLNode>::iterator _iter;
	...
	while(tempCnt--)
	{
		...
		{
			if(!wcscmp(_iter->getName(),str)) searchNodeQ.push(&(*_iter));
		}
		searchNodeQ.pop();
	}
}
